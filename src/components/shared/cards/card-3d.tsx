...
const Card3D = forwardRef<HTMLDivElement, Props>(
  ({ description, image, tags, title, url, className, ...props }, ref) => {
    return (
      <CardContainer
        className={cn('h-full rounded-lg bg-card p-4 shadowCard', className)}
        ref={ref}
        {...props}
      >
        <CardBody className="flex h-full flex-col">
          ...
          <CardItem className="mb-4 mt-1 max-xl:text-sm" as="p" translateZ={20}>
            {description}
          </CardItem>
